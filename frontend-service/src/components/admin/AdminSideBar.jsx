...
const AdminSideBar = ({ activeView, setActiveView }) => {
    const navItems = [
        { id: 'accounts', icon: User, label: 'Quản lý Tài khoản' },
    ];
    ...
    return (
        <div className="w-60 bg-gray-900 text-white flex flex-col min-h-screen">
            <div className="flex grid mb-[50px] items-center justify-center h-16 my-5 ">
                <img className='h-auto w-35 mb-[10px]' src="https://www.galaxycine.vn/_next/static/media/icon-login.fbbf1b2d.svg" alt="" />
                <h1 className="text-xl font-bold ms-0">Admin Panel</h1>
            </div>
            <nav className="flex-1 mt-5 px-2 py-4 overflow-y-auto">
                <ul>
                    {navItems.map(item => (
                        <li className={`py-1 ${activeView === item.id ? 'bg-red-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`} key={item.id}>
                            <button
                                onClick={() => setActiveView(item.id)}
                                className={`w-full flex items-center px-2 py-2 text-left rounded-lg transition-colors duration-200 ${activeView === item.id
