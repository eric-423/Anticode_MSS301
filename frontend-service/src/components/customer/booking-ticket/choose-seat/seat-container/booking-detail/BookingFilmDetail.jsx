...
import {
  getSeatNamesByShowtime,
  getSeatsByShowtime,
  getSelectedSeatsDetail,
  saveSelectedSeatsDetail,
} from '../../../../../../utils/seatStorage'
...
const BookingFilmDetail = () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        const availableSeatsForStudent = currentSeats.filter((seat) => {
          ...
          console.log(
            `Ghế ${seat.seatName}: hàng ${seatRow}, tổng hàng ${totalRows}, là hàng cuối: ${isLastRow}`
          )
        ...
        if (availableSeatsForStudent.length > 0) {
          ...
          if (
            ...
            currentSeats.length === lastRowSeats.length
          ) {
            alert(
              'Ưu đãi sinh viên không áp dụng cho ghế hàng cuối. Vui lòng chọn thêm ghế khác để áp dụng ưu đãi.'
            )
          } else {
            alert(
              'Đã áp dụng ưu đãi học sinh. Vui lòng chọn ít nhất 1 chỗ ngồi (không phải hàng cuối) để áp dụng ưu đãi.'
            )
          }
        }
      }
    ...
    if (!isEnabled) {
      ...
      const updatedSeatsDetail = allSeatsDetail.map((seat) => {
        if (seat.showtime === parseInt(showtimeId)) {
          ...
          const isLastRow =
            seat.seatName.charCodeAt(0) - 65 ===
            (showtime?.cinemaHall?.hallType?.roll || 0) - 1
          return {
            ...seat,
      ...
      if (seat.showtime === parseInt(showtimeId)) {
        if (seat.seatName === selectedSeat) {
          return {
            ...seat,
            ticketType: 3,
            originalPrice: seat.originalPrice || seat.price,
            price: 45000,
          }
        } else {
          const isLastRow =
          ...
          return {
            ...seat,
            ticketType: isLastRow ? 2 : 1,
            price: seat.originalPrice || seat.price,
  ...
  const calculateCurrentShowtimeTotal = () => {
    ...
    return currentShowtimeSeats.reduce(
  ...
  return (
    <div>
      <div
        className="h-[6px] bg-primary rounded-t-lg"
        style={{
      ...
      <div className="gap-2 bg-white p-4 rounded-lg shadow-md">
        <div className="flex item-start gap-2">
          ...
          <div className="flex-1 col-span-2 md:col-span-1 row-span-1 xl:col-span-2">
            <h3 className="text-[17px] font-semibold text-[#4A4A4A] mb-2">
              {movie?.title}
            </h3>
            ...
            <div className="xl:mt-2 ml-2 xl:ml-0 inline-block">
              <span
                className="inline-flex items-center justify-center w-[38px] h-7 rounded text-sm text-center"
                style={{
                  color: 'white',
                  background: 'rgb(245, 128, 32)',
                }}
        ...
        <div className="xl:mt-4 flex items-center gap-4">
          ...
          <div
            className={`transition-all duration-300 flex-1 ${
              isUploadEnabled ? 'opacity-100' : 'opacity-50'
            }`}
          >
            <input
              type="file"
              ...
              className={`block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100 ${
                !isUploadEnabled ? 'pointer-events-none' : ''
              }`}
      ...
      <div className="mt-8 xl:flex hidden justify-between">
        <button className="w-1/2 mr-2 py-2 text-primary" onClick={handleBack}>
          <span
            style={{
              color: 'rgb(245, 128, 32)',
              cursor: 'pointer',
        ...
        <div className="fixed inset-0 bg-[#00000080] bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-md max-w-md w-full mx-4">
            <h2 className="text-xl font-semibold mb-4 text-center">
              Chọn ghế áp dụng ưu đãi học sinh
            </h2>
            <p className="text-sm text-gray-600 mb-4 text-center">
              Bạn đã chọn {selectedSeatsForStudent.length} ghế (không bao gồm
              ghế hàng cuối). Vui lòng chọn 1 ghế để áp dụng ưu đãi học sinh
              (giảm 20% giá vé):
            </p>
